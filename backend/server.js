...
// On Render with rootDir:backend, __dirname IS the backend folder
// data.json lives right next to server.js
const DB_PATH = path.join(__dirname, 'data.json');

// Frontend is at ../frontend/public relative to backend/
// But on Render, the whole repo is cloned and rootDir only affects
// where npm install + start run — the full repo is still available
// We serve from a path that works both locally and on Render
...
function initDB() {
  ...
  const products = [
    ...
    { id:12, name:"Reese's Cups",             category:"Candy",         price:1.25, image:"🍫", description:"Peanut butter & chocolate" },
